...
static int extract_and_install(int fd, bool dry_run, struct swupdate_cfg *software) {
...
int install_from_file(const char *filename, bool check, struct swupdate_cfg *software)
{
	...
	* (flash images) from the install image and
	* install into flash.
	*/
	ret = extract_and_install(fd, check, software);
